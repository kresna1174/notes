...
export function EditDiagramDialog({
  ...
  async function generateWithAi() {
    ...
    try {
      ...
      if (aiPromptTarget) {
        finalPrompt = `Modify/develop this node in the diagram.
Node Target: { id: "${aiPromptTarget.id}", label: "${aiPromptTarget.data?.label || ''}" }
User Instructions: ${aiPrompt}
Current Diagram: ${JSON.stringify({ nodes, edges })}`
      } else if (aiCoords) {
        finalPrompt = `${aiPrompt}
Note: Position the main/root node at coordinates (x: ${Math.round(aiCoords.x)}, y: ${Math.round(aiCoords.y)}).`
      }
      ...
      alert('Failed to create diagram: ' + String(err))
    } finally {
      setAiLoading(false)
    }
  }
      ...
      >
        <DialogHeader style={{ padding: '16px 20px 12px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <DialogTitle style={{ fontSize: '0.9375rem', color: 'var(--fg)', fontFamily: 'var(--font-heading)' }}>
            Edit Diagram (Double-click node to change label)
          </DialogTitle>
        </DialogHeader>
            ...
            {contextMenu.node ? (
              <>
                <div style={{ padding: '4px 8px', color: 'var(--fg-muted)', fontSize: '0.7rem', fontWeight: 600, borderBottom: '1px solid var(--border)', marginBottom: '4px' }}>
                  Node: {contextMenu.node.data?.label || 'Untitled'}
                </div>
                <button
                  onClick={() => {
          ...
          <div style={{
            ...
            <div style={{
              ...
              >
                ...
                <span style={{ cursor: 'inherit', flex: 1 }}>Create Diagram with AI</span>
              </div>
              
              <p style={{ fontSize: '0.75rem', color: 'var(--fg-muted)', margin: 0 }}>
                Describe the diagram you want to create. AI will automatically design nodes, relationship flows, coordinates, and layout for you.
              </p>
              ...
              {aiLoading ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '20px 0' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: 'var(--fg-muted)' }}>AI is designing the diagram...</span>
                </div>
              ) : (
                <>
                  <textarea
                    autoFocus
                    placeholder="Example: New user registration flow with email OTP verification, if successful go to dashboard, if failed return to form..."
                    value={aiPrompt}
        ...
        {showRenameInput && renameCoords && (
          <div style={{
            ...
            <div style={{
              ...
              >
                <GripVertical size={14} style={{ color: 'var(--fg-muted)', opacity: 0.5, cursor: 'inherit' }} />
                <span style={{ cursor: 'inherit', flex: 1 }}>Change Node Label</span>
              </div>
