...
export function ShareModal({ noteId, initialToken, initialHasPin, onClose, onShareChange, onActionSuccess, isTeamNote }: Props) {
  ...
  async function removeLink() {
    if (!window.confirm('Delete public sharing link?')) return
    setLoading(true)
  ...
  async function handleCopyToOrg() {
    ...
    if (teamPinEnabled && (!teamPin || !/^\d{4}$/.test(teamPin))) {
      setTeamPinError('PIN must be 4 digits')
    ...
    try {
      ...
      const data = await r.json()
      if (r.ok) {
        setActionMessage({ text: 'Note copied to organization!', type: 'success' })
        setTimeout(() => {
          onActionSuccess?.()
          onClose()
        }, 1200)
      } else {
        setActionMessage({ text: data.error || 'Failed to copy note', type: 'error' })
      }
    } catch {
      setActionMessage({ text: 'A network error occurred', type: 'error' })
    } finally {
  ...
  async function handleMoveToOrg() {
    ...
    try {
      ...
      const data = await r.json()
      if (r.ok) {
        setActionMessage({ text: 'Note moved to organization!', type: 'success' })
        setTimeout(() => {
          onActionSuccess?.()
          onClose()
        }, 1200)
      } else {
        setActionMessage({ text: data.error || 'Failed to move note', type: 'error' })
      }
    } catch {
      setActionMessage({ text: 'A network error occurred', type: 'error' })
    } finally {
  ...
  async function handleCopyToPersonal() {
    ...
    try {
      ...
      const data = await r.json()
      if (r.ok) {
        setActionMessage({ text: 'Note copied to personal notes!', type: 'success' })
        setTimeout(() => {
          onActionSuccess?.()
          onClose()
        }, 1200)
  ...
  async function handleMoveToPersonal() {
    ...
    try {
      ...
      const data = await r.json()
      if (r.ok) {
        setActionMessage({ text: 'Note moved to personal notes!', type: 'success' })
        setTimeout(() => {
          onActionSuccess?.()
          onClose()
        }, 1200)
    ...
    >
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <p style={{ margin: 0, fontWeight: 700, fontSize: '1rem', color: 'var(--fg)', fontFamily: 'var(--font-heading)' }}>Share Note</p>
            <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--fg-muted)', fontFamily: 'var(--font-body)' }}>Share this note publicly or with an organization</p>
          </div>
        </div>
        ...
        {user?.organizations && user.organizations.length > 0 && (
          <div style={{ display: 'flex', gap: 4, background: 'var(--muted)', borderRadius: 8, padding: 3, marginBottom: 20 }}>
            ...
            >
              {isTeamNote ? 'Personal' : 'Organization'}
            </button>
          </div>
        )}
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div>
                  <p style={{ margin: 0, fontSize: '0.8125rem', fontWeight: 600, color: 'var(--fg)', fontFamily: 'var(--font-body)' }}>PIN Protection</p>
                  <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--fg-muted)', fontFamily: 'var(--font-body)' }}>
                    {pinEnabled ? 'Requires 4-digit PIN to view note' : 'Anyone with the link can view'}
                  </p>
                </div>
              </div>
              <button
            ...
            {shareUrl ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    ...
                    title="Refresh Link (Generate New Token)"
                    style={{
              ...
              >
                {loading ? 'Creating link...' : 'Enable Public Sharing'}
              </button>
            )}
          </>
        ) : (
          user?.organizations && user.organizations.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p style={{ margin: 0, fontSize: '0.8125rem', color: 'var(--fg-muted)', lineHeight: 1.4, fontFamily: 'var(--font-body)' }}>
                {isTeamNote
                  ? 'Pull this note to your personal notes. You can copy this note (create a new copy in the Personal tab) or move it completely to your personal workspace.'
                  : 'Share this note with your organization. You can copy this note (create a new copy in the Organization tab) or move it completely to the organization workspace.'}
              </p>
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <div>
                        <p style={{ margin: 0, fontSize: '0.8125rem', fontWeight: 600, color: 'var(--fg)', fontFamily: 'var(--font-body)' }}>Organization PIN Protection</p>
                        <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--fg-muted)', fontFamily: 'var(--font-body)' }}>
                          {teamPinEnabled ? 'Requires PIN to open this organization note' : 'All organization members can read directly'}
                        </p>
                      </div>
                    </div>
                    <button
              ...
              <label style={{ 
                ...
                <span>
                  {isTeamNote 
                    ? 'I agree to move this note to personal space (note will be removed from Organization)'
                    : 'I agree to move this note to Organization space (note will be removed from Personal notes)'}
                </span>
              </label>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                ...
                >
                  {actionLoading === 'copy' ? 'Copying...' : (isTeamNote ? 'Copy to Personal' : 'Copy to Org')}
                </button>
                <button
                ...
                >
                  {actionLoading === 'move' ? 'Moving...' : (isTeamNote ? 'Move to Personal' : 'Move to Org')}
                </button>
              </div>
              {actionMessage && (
                <p style={{
