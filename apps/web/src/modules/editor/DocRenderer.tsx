        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ margin: '2px 0 0', fontSize: '0.72rem', color: 'var(--fg-muted,#6c757d)' }}>
              {nodes.length} node · {edges.length} connections
            </p>
          </div>
        </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              boxShadow: '0 1px 4px rgba(0,0,0,0.05)'
               ...
               Drag to pan {isWide && '· Pinch to zoom'}
            </div>
...
function TextDocPreview({ url, filename }: { url: string; filename: string }) {
  ...
  if (loading) return (
    <div style={{ padding: '16px', fontSize: '0.8rem', color: 'var(--fg-muted, #6c757d)', fontFamily: 'var(--font-body, sans-serif)' }}>Loading…</div>
  )
        ...
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {canPreview && (
            ...
            >
              ...
              {expanded ? 'Hide' : isImage ? 'View Image' : 'View Document'}
            </button>
          )}
