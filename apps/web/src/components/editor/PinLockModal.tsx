...
export function PinLockModal({ mode, onSubmit, onClose }: Props) {
  ...
  const title = mode === 'unlock' ? 'Masukkan PIN' : mode === 'set' ? 'Buat PIN Baru' : 'Masukkan PIN untuk Hapus'
  const subtitle = mode === 'set' ? '4 digit angka untuk mengunci catatan ini' : 'Masukkan 4 digit PIN'
