...
def search_knowledge(query: str, n_results: int = 5) -> str:
    ...
    try:
        ...
        seen_notes = {}
        for kp, meta, score, cosine in note_hits:
            ...
            seen_notes[note_id]["kps"].append((kp, score, cosine))

        for note_id, note_data in seen_notes.items():
            kps_lines = []
            for kp, score, cosine in note_data["kps"]:
                rel = f", Relevance: {cosine * 100:.0f}%" if cosine is not None else ""
                kps_lines.append(f"- {kp} (Score: {score:.4f}{rel})")
            kps_str = "\n".join(kps_lines)
            output.append(
